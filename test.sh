...
popd
pushd ./opts
  g++ ${BASE_OPT} -I/usr/local/opt/openssl/include -L/usr/local/opt/openssl/lib -lboost_filesystem -lssl -lcrypto -ldl -lyaml-cpp -DPOAC_USE_GITHUB_TOKEN_FROM_ENV -DPOAC_VERSION=\"0.2.1\" -DPOAC_PROJECT_ROOT=\"${ROOT_DIR}\" -o publish-test publish.cpp && { ./publish-test; rm -rf ./publish-test; }
popd
pushd ./util
