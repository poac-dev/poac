...
auto
split(const String& raw, const String& delim) -> Vec<String> {
  Vec<String> ret;
...
auto
dupenv(const String& name) -> Option<String> {
#if BOOST_COMP_MSVC
  char* env;
  usize len;
...
auto
getenv(const String& name, const String& default_v) -> String {
...
[[nodiscard]] auto
expand_user() -> Result<Path, String> {
