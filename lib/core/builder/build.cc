...
auto
operator<<(std::ostream& os, Mode mode) -> std::ostream& {
  switch (mode) {
    case Mode::debug:
      return (os << "dev");
    case Mode::release:
...
[[nodiscard]] auto
run(data::NinjaMain& ninja_main, Status& status) -> Result<void> {
  String err;
...
auto
get_ninja_verbosity() -> BuildConfig::Verbosity {
