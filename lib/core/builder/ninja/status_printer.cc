...
StatusPrinter::StatusPrinter(
    const BuildConfig& config, StringRef progress_status_format
)
...
void
StatusPrinter::BuildEdgeStarted(const Edge* edge, i64 start_time_millis) {
  ++started_edges_;
  ++running_edges_;
...
void
StatusPrinter::PrintStatus(const Edge* edge, i64 time_millis) {
  if (config_.verbosity == BuildConfig::QUIET ||
...
void
StatusPrinter::SlidingRateInfo::UpdateRate(int update_hint, i64 time_millis_) {
