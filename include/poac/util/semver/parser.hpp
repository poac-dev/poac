...
struct Parser {
  ...
  std::optional<std::uint_fast64_t> dot_component();

  /// Parse a dot, then a numeric.
  std::optional<std::uint_fast64_t> dot_numeric();
  ...
  std::vector<Identifier> plus_build_metadata();
  ...
  inline bool is_eof() const { return lexer.size() < lexer.c1_index; }
...
Version parse(std::string_view input);
