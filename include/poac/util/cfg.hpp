...
struct operator_error : public cfg::exception {
  public:
    ...
    explicit operator_error(const char* what_)
        : operator_error(String(what_)) {}
...
struct CfgExpr {
    ...
    using variant_type =
        std::variant<null_type, expr_type, expr_list_type, Cfg>;
      ...
      if (const auto token = lexer.peek(); !token.has_value()) {
        ...
        if (token->get_ident() == Token::ident::all ||
          ...
          if (token->get_ident() == Token::ident::not_) {
            ...
            return CfgExpr{
                CfgExpr::cfg, std::make_unique<CfgExpr>(std::move(e))};
          }
        }
      }
