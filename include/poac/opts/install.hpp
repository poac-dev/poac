...
namespace poac::opts::install {
    ...
    void stream_deps(YAML::Emitter& out, const core::resolver::resolve::DuplicateDeps& deps) { // TODO: Activatedは，重複を許容しているのに，ここでは，許容する意味は無い．-> よって，Activated型は不適
    ...
    void create_lockfile(const std::string& timestamp, const core::resolver::resolve::DuplicateDeps& activated_deps) {
        // Create a poac.lock
    ...
    fetch(const core::resolver::resolve::NoDuplicateDeps& deps, const install::Options& opts) {
        ...
        for (const auto& [name, version] : deps) {
            ...
            if (core::resolver::resolve::current::resolve(current_name)) {
                ...
                util::shell clone_cmd = core::resolver::resolve::github::clone_command(name, version);
