...
//                stream_deps(out, package.dependencies.value()); // FIXME
    ...
    void create_lockfile(const std::string& timestamp, const core::resolver::resolve::Activated& activated_deps) {
        // Create a poac.lock
    ...
    void fetch_packages(
        ...
        for (const auto& [name, version] : deps) {
            ...
            const std::string cache_name = core::name::to_cache(name, version);
            ...
            if (resolve::current::resolve(current_name)) {
                ...
                if (!opts.quite) {
                    echo_install_status(res, name, version);
                }
            }
            else {
                util::shell clone_cmd = resolve::github::clone_command(name, version);
                ...
                if (!opts.quite) {
                    echo_install_status(result, name, version);
                }
            }
        }
    ...
    std::optional<io::yaml::Lockfile> // TODO:
...
//        }
        ...
        fetch_packages(resolved_deps.backtracked, opts);
        if (!opts.quite) {
            std::cout << std::endl;
            io::term::status_done();
        }
