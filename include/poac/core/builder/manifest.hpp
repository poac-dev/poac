...
inline fs::file_time_type ninja_manifest_last_modified(const Path& build_dir) {
  return fs::last_write_time(build_dir / manifest_file_name);
}

inline bool is_outdated(const Path& build_dir) {
...
bool rebuild(data::NinjaMain& ninja_main, Status& status, String& err);

Vec<String> gather_includes(const resolver::ResolvedDeps& resolved_deps);

Vec<toml::table> get_cfg_profile(const toml::value& poac_manifest);

Vec<String> gather_flags(
    const toml::value& poac_manifest, const String& name,
    const Option<String>& prefix = None
);

[[nodiscard]] Result<String> construct(
