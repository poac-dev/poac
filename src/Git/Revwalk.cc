...
Revwalk& Revwalk::setSorting(unsigned int sort_mode) {
...
Revwalk& Revwalk::push(const Oid& oid_) {
...
Revwalk& Revwalk::pushGlob(const StringRef glob) {
  git2Throw(git_revwalk_push_glob(this->raw, glob.data()));
  return *this;
}

Revwalk& Revwalk::pushRange(const StringRef range) {
  git2Throw(git_revwalk_push_range(this->raw, range.data()));
  return *this;
}

Revwalk& Revwalk::pushRef(const StringRef reference) {
...
Revwalk& Revwalk::hideGlob(const StringRef glob) {
  git2Throw(git_revwalk_hide_glob(this->raw, glob.data()));
  return *this;
}

Revwalk& Revwalk::hideRef(const StringRef reference) {
  git2Throw(git_revwalk_hide_ref(this->raw, reference.data()));
  return *this;
}

Oid Revwalk::next() const {
  git_oid oid;
