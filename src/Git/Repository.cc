...
Repository& Repository::open(const StringRef path) {
  git2::init();
  git2Throw(git_repository_open(&this->raw, path.data()));
  return *this;
}
Repository& Repository::openBare(const StringRef path) {
  git2::init();
  git2Throw(git_repository_open_bare(&this->raw, path.data()));
  return *this;
}

Repository& Repository::init(const StringRef path) {
  git2::init();
  git2Throw(git_repository_init(&this->raw, path.data(), false));
  return *this;
}
Repository& Repository::initBare(const StringRef path) {
  git2::init();
  git2Throw(git_repository_init(&this->raw, path.data(), true));
  return *this;
}

bool Repository::isIgnored(const StringRef path) const {
  git2::init();
