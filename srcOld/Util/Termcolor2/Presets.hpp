...
inline constexpr auto gray_v() noexcept -> std::basic_string<CharT> {
  return CHAR_LITERAL(CharT, "\033[30m");
}
inline const auto gray = gray_v();

template <typename CharT = char>
inline constexpr auto red_v() noexcept -> std::basic_string<CharT> {
  return CHAR_LITERAL(CharT, "\033[31m");
}
inline const auto red = red_v();

template <typename CharT = char>
inline constexpr auto green_v() noexcept -> std::basic_string<CharT> {
  return CHAR_LITERAL(CharT, "\033[32m");
}
inline const auto green = green_v();

template <typename CharT = char>
inline constexpr auto yellow_v() noexcept -> std::basic_string<CharT> {
  return CHAR_LITERAL(CharT, "\033[33m");
}
inline const auto yellow = yellow_v();

template <typename CharT = char>
inline constexpr auto blue_v() noexcept -> std::basic_string<CharT> {
  return CHAR_LITERAL(CharT, "\033[34m");
}
inline const auto blue = blue_v();

template <typename CharT = char>
inline constexpr auto magenta_v() noexcept -> std::basic_string<CharT> {
  return CHAR_LITERAL(CharT, "\033[35m");
}
inline const auto magenta = magenta_v();

template <typename CharT = char>
inline constexpr auto cyan_v() noexcept -> std::basic_string<CharT> {
  return CHAR_LITERAL(CharT, "\033[36m");
}
inline const auto cyan = cyan_v();

template <typename CharT = char>
inline constexpr auto white_v() noexcept -> std::basic_string<CharT> {
...
//
template <typename CharT = char>
inline constexpr auto on_gray_v() noexcept -> std::basic_string<CharT> {
  return CHAR_LITERAL(CharT, "\033[40m");
}
inline const auto on_gray = on_gray_v();

template <typename CharT = char>
inline constexpr auto on_red_v() noexcept -> std::basic_string<CharT> {
  return CHAR_LITERAL(CharT, "\033[41m");
}
inline const auto on_red = on_red_v();

template <typename CharT = char>
inline constexpr auto on_green_v() noexcept -> std::basic_string<CharT> {
  return CHAR_LITERAL(CharT, "\033[42m");
}
inline const auto on_green = on_green_v();

template <typename CharT = char>
inline constexpr auto on_yellow_v() noexcept -> std::basic_string<CharT> {
  return CHAR_LITERAL(CharT, "\033[43m");
}
inline const auto on_yellow = on_yellow_v();

template <typename CharT = char>
inline constexpr auto on_blue_v() noexcept -> std::basic_string<CharT> {
  return CHAR_LITERAL(CharT, "\033[44m");
}
inline const auto on_blue = on_blue_v();

template <typename CharT = char>
inline constexpr auto on_magenta_v() noexcept -> std::basic_string<CharT> {
  return CHAR_LITERAL(CharT, "\033[45m");
}
inline const auto on_magenta = on_magenta_v();

template <typename CharT = char>
inline constexpr auto on_cyan_v() noexcept -> std::basic_string<CharT> {
  return CHAR_LITERAL(CharT, "\033[46m");
}
inline const auto on_cyan = on_cyan_v();

template <typename CharT = char>
inline constexpr auto on_white_v() noexcept -> std::basic_string<CharT> {
...
//
template <typename CharT = char>
inline constexpr auto bold_v() noexcept -> std::basic_string<CharT> {
  return CHAR_LITERAL(CharT, "\033[01m");
}
inline const auto bold = bold_v();

template <typename CharT = char>
inline constexpr auto dark_v() noexcept -> std::basic_string<CharT> {
  return CHAR_LITERAL(CharT, "\033[02m");
}
inline const auto dark = dark_v();

template <typename CharT = char>
inline constexpr auto underline_v() noexcept -> std::basic_string<CharT> {
  return CHAR_LITERAL(CharT, "\033[04m");
}
inline const auto underline = underline_v();

template <typename CharT = char>
inline constexpr auto blink_v() noexcept -> std::basic_string<CharT> {
  return CHAR_LITERAL(CharT, "\033[05m");
}
inline const auto blink = blink_v();

template <typename CharT = char>
inline constexpr auto reverse_v() noexcept -> std::basic_string<CharT> {
  return CHAR_LITERAL(CharT, "\033[07m");
}
inline const auto reverse = reverse_v();

template <typename CharT = char>
inline constexpr auto concealed_v() noexcept -> std::basic_string<CharT> {
  return CHAR_LITERAL(CharT, "\033[08m");
}
inline const auto concealed = concealed_v();

template <typename CharT = char>
inline constexpr auto reset_v() noexcept -> std::basic_string<CharT> {
