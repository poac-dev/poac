...
export module poac.util.registry.conan.v1.resolver;
...
auto check_conan_command() -> Result<void> {
...
auto install_conan_generator() -> Result<void> {
...
auto format_conan_requires(const Vec<core::resolver::resolve::Package>& packages
) -> String {
  Vec<String> lines;
...
auto generate_conanfile(const Vec<core::resolver::resolve::Package>& packages)
    -> Result<void> {
...
export auto
fetch_conan_packages(const Vec<core::resolver::resolve::Package>& packages
...
export auto is_conan(const core::resolver::resolve::Package& package) noexcept
    -> bool {
