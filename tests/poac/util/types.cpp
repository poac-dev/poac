...
BOOST_AUTO_TEST_CASE( poac_util_types_duplicate_test )
{
    ...
    BOOST_TEST( duplicate(test_case) );
...
BOOST_AUTO_TEST_CASE( poac_util_types_ptree_to_vector_test )
{
    ...
    BOOST_TEST( ptree_to_vector<std::string>(pt, "data") == test_case ); // 1
    BOOST_TEST( ptree_to_vector<std::string>(children) == test_case ); // 2
}
