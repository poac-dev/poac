...
int main() {
    ...
    "test parser::Token"_test = [] {
        ...
        }
        {
            ...
            expect(constant<token.component == Token::variant_type{std::make_pair(0, 2)}>);
        }
        {
            constexpr Token token{ Token::Numeric, 0 };
            expect(constant<token.kind == Token::Numeric>);
            expect(constant<token.component == Token::variant_type{std::uint_fast64_t{0}}>);
        }
        {
    ...
    "test token_eq"_test = [] {
        ...
        expect(constant<Token( Token::Whitespace, 0, 2 ) == Token( Token::Whitespace, 0, 2 )>);
        expect(constant<Token( Token::Numeric, 0 ) == Token( Token::Numeric, 0 )>);
        expect(constant<Token( Token::AlphaNumeric, "x") == Token( Token::AlphaNumeric, "x")>);
    };
    ...
    "test token_neq"_test = [] {
        ...
        expect(constant<Token( Token::Numeric, 0 ) != Token( Token::AlphaNumeric, "0")>);
    };
}
