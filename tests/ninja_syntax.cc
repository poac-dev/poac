...
main() {
  ...
  describe("test expand") = [] {
    it("test basic") = [] {
      const ninja::syntax::Variables vars{{"x", "X"}};
      expect(eq("foo"s, ninja::syntax::expand("foo", vars)));
    };

    it("test var") = [] {
      const ninja::syntax::Variables vars{{"xyz", "XYZ"}};
      expect(eq("fooXYZ"s, ninja::syntax::expand("foo$xyz", vars)));
    };

    it("test vars") = [] {
      const ninja::syntax::Variables vars{{"x", "X"}, {"y", "YYY"}};
      expect(eq("XYYY"s, ninja::syntax::expand("$x$y", vars)));
    };

    it("test space") = [] {
      const ninja::syntax::Variables vars{};
      expect(eq("x y z"s, ninja::syntax::expand("x$ y$ z", vars)));
    };

    it("test locals") = [] {
      const ninja::syntax::Variables vars{{"x", "a"}};
      const ninja::syntax::Variables local_vars{{"x", "b"}};
